    ...
    def admin_mutate(self,actor,action,values):
        ...
        with self._actor_transaction(actor) as c:
            ...
            if action in ('suspend','unsuspend','grant'):
                ...
                cfg=configuration({'BILLING_ENABLED':'true','OWNER_TELEGRAM_ID':str(actor),'SUBSCRIPTION_STARS':values.get('stars',''),'TRIAL_DAYS':values.get('trial_days','7'),'TEXT_MONTHLY_QUOTA':values.get('text_quota',''),'VOICE_MONTHLY_SECONDS':values.get('voice_seconds',''),'IMAGE_MONTHLY_QUOTA':values.get('image_quota',''),'ANALYSIS_PERIOD_QUOTA':values.get('analysis_quota','30'),'TRIAL_TEXT_QUOTA':values.get('trial_text_quota','100'),'TRIAL_VOICE_SECONDS':values.get('trial_voice_seconds','300'),'TRIAL_IMAGE_QUOTA':values.get('trial_image_quota','10'),'TRIAL_ANALYSIS_QUOTA':values.get('trial_analysis_quota','3'),'TERMS_URL':values.get('terms_url',''),'SUPPORT_CONTACT':values.get('support_contact','')})
