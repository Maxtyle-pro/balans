...
def create_app(service,bot=None):
    ...
    async def home(request):
        ...
        if section=='users':
            ...
            row=rows[0];f='<h2>Новая версия тарифа</h2>'+form('tariff',''.join(field(k,label,'number' if k in ('stars','trial_days','text_quota','voice_seconds','image_quota','analysis_quota','trial_text_quota','trial_voice_seconds','trial_image_quota','trial_analysis_quota') else 'text',row[k] or '') for k,label in [('stars','Цена Stars'),('trial_days','Пробный период, дней'),('text_quota','ИИ-категоризаций / 30 дней'),('voice_seconds','Секунд голоса / 30 дней'),('image_quota','Файлов чеков / 30 дней'),('analysis_quota','ИИ-анализов / 30 дней'),('trial_text_quota','ИИ-категоризаций / пробный период'),('trial_voice_seconds','Секунд голоса / пробный период'),('trial_image_quota','Файлов чеков / пробный период'),('trial_analysis_quota','ИИ-анализов / пробный период'),('terms_url','Опубликованные условия HTTPS'),('support_contact','Контакт поддержки')]),s,'Сохранить и включить оплату')
