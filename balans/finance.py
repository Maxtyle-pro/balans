...
class Finance:
    ...
    def _finance_callback(self,c,user,callback,sent):
        ...
        if action in ('fedit','fdelete','frefund','faudit'):
            ...
            if action=='faudit':
                rows=c.execute('SELECT revision_no,amount,currency,description,occurred_on,change_reason FROM operation_revisions WHERE operation_id=%s ORDER BY revision_no DESC LIMIT 10',(o['operation_id'],)).fetchall()
                return Reply('История изменений (последние 10)\n'+'\n\n'.join(f"Версия {r['revision_no']} · {r['occurred_on']} · {money(r['amount'],r['currency'])}\n{r['description'][:100]}\n{r['change_reason'] or '—'}" for r in rows))
        ...
        if not d or d['state']=='cancelled':return Reply('Черновик недоступен или карточка устарела.')
        if d['state']=='saved':return Reply('Операция уже сохранена, повторной записи нет.')
        if action=='ffield':
