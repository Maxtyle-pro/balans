...
def migrate(dsn: str, runtime_role: str) -> None:
    with psycopg.connect(dsn) as conn:
        ...
        conn.execute(sql.SQL("GRANT INSERT, UPDATE ON balans.text_jobs,balans.voice_jobs TO {}").format(role))
        ...
        conn.execute(sql.SQL("GRANT INSERT ON balans.addon_orders TO {}").format(role))
        conn.execute(sql.SQL("GRANT EXECUTE ON FUNCTION balans.plan_retention_notices(),balans.mark_retention_notice(uuid),balans.confirm_demo_addon(uuid) TO {}").format(role))
        conn.execute(sql.SQL("GRANT EXECUTE ON FUNCTION balans.choose_user_currency(text) TO {}").format(role))
        conn.execute(sql.SQL("GRANT UPDATE(automatic_capture) ON balans.user_settings TO {}").format(role))
        conn.execute(sql.SQL("GRANT EXECUTE ON FUNCTION balans.uncategorized_category(uuid) TO {}").format(role))
