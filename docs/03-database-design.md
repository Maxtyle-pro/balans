...
Сроки из ТЗ: голос — 24 часа после обработки, личные изображения и shared_evidence — 90 дней с возможностью продления по отдельной политике. Отмена записи не запускает немедленное удаление общего доказательного файла. Пока существует хотя бы одна сохраняемая ссылка на файл, его физическое удаление не выполняется. Для исходников зависших задач задать отдельный предельный срок обработки/очистки.
...
Сохранение draft блокирует его строку и проверяет version. Повтор возвращает уже созданную операцию. Один успешный draft имеет одну операцию, включая передачу/заявление. Связь saved_operation_id проверяется на обратное соответствие operations.source_draft_id. Для черновика исправления `edit_origin_message_id` и `edit_message_ids` хранят только технические Telegram ID: после отправки нового значения временные сообщения редактора удаляются, а исходная карточка редактируется на месте; финансовая история ими не заменяется и не удаляется. Отдельный вопрос о сохранении для пользовательского исправления не используется.
...
| Чек из терминала «К оплате» | В автоматическом режиме — расход после распознавания; при явном failed — только draft до проверки |
